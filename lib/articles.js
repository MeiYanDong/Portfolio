...
export function getAllSeries() {
  ...
  return seriesData
    .map((series) => {
      ...
      const latestArticle = [...seriesArticles].sort(
...
function flushList(listItems, listType, html, slug) {
  ...
  const type = listType.current
  let listHtml = `<${type}>`
  let rootItemOpen = false
  let nestedListOpen = false

  listItems.forEach((item) => {
    if (item.depth > 0 && rootItemOpen) {
      if (!nestedListOpen) {
        listHtml += `<${type}>`
        nestedListOpen = true
      }
      listHtml += `<li>${inlineMarkdown(item.content, slug)}</li>`
      return
    }
    ...
    if (rootItemOpen) listHtml += '</li>'
    listHtml += `<li>${inlineMarkdown(item.content, slug)}`
    rootItemOpen = true
  })

  if (nestedListOpen) listHtml += `</${type}>`
  if (rootItemOpen) listHtml += '</li>'
  html.push(`${listHtml}</${type}>`)
  listItems.length = 0
...
export function getArticleHeadings(markdown) {
  ...
  markdown
    ...
    .forEach((rawLine) => {
      ...
      const id = createHeadingId(heading[2], usedIds)
      if (level < 2) return
      headings.push({
        id,
        level,
...
export function markdownToHtml(markdown, slug) {
  ...
  lines.forEach((rawLine) => {
    ...
    if (image) {
      ...
      const caption =
        image[1] && !isGeneratedImageAlt(image[1]) ? `<figcaption>${alt}</figcaption>` : ''
    ...
    if (heading) {
      ...
      flushList(listItems, listType, html, slug)
      const level = heading[1].length
      const id = createHeadingId(heading[2], usedHeadingIds)
      html.push(`<h${level} id="${escapeHtml(id)}">${inlineMarkdown(heading[2], slug)}</h${level}>`)
      return
    }
    ...
    if (listItem) {
      flushParagraph(paragraphLines, html, slug)
      const nextType = /^[-*]$/.test(listItem[2]) ? 'ul' : 'ol'
      ...
      listItems.push({
        content: listItem[3],
        depth: listItem[1].replace(/\t/g, '    ').length > 0 ? 1 : 0
      })
      return
    }
